    ...
    with open(ori_data_file) as ori_data_file_handle:
        for line in ori_data_file_handle:
            if dataset_type == 'train':
                ...
                new_data_file_handle.write(f'{edge_type}\t{reindex_dic[left]}\t{reindex_dic[right]}\t{1}\n')
                neg_left, neg_right = int(random.random() * node_num[data_name]), int(
                    random.random() * node_num[data_name])
                new_data_file_handle.write(f'{edge_type}\t{neg_left}\t{neg_right}\t{0}\n')
            else:
                edge_type, left, right, label = line[:-1].split(' ')
                new_data_file_handle.write(
                    f'{edge_type}\t{reindex_dic[left]}\t{reindex_dic[right]}\t{label}\n')
...
def gen_reindex_dic():
    ...
    for ori_index in ori_index_list:
        ...
        reindex += 1
    print(f'reindex {reindex} nodes and write in to {reindex_dic_file}')
    return


if not os.path.exists(reindex_dic_file):
    gen_reindex_dic()
