...
class data_loader:
    def __init__(self, path, edge_types=[]):
        ...
        self.links, self.links_test, self.train_pos, self.valid_pos = self.load_links(
            is_test=False)
    ...
    def evaluate(edge_list, confidence, labels):
        ...
        mrr_list, cur_mrr = [], 0
        t_dict, labels_dict, conf_dict = defaultdict(
    ...
    def load_types(self):
        ...
        entities_path = os.path.join(self.path, 'entities.dict')
        entities = read_file.read_dictionary(entities_path)
        for i in entities:
            node_id = int(i)
            node_type = 0
    ...
    def get_train_neg(self, edge_types=[]):
        ...
        for r_id in edge_types:
            ...
            t_range = (self.nodes['shift'][t_type], self.nodes['shift']
                       [t_type] + self.nodes['count'][t_type])
            '''get neg_neigh'''
            train_neg[r_id] = [[], []]
    ...
    def get_test_neigh(self):
        ...
        for r_id in self.links_test['data'].keys():
            pos_links += self.links_test['data'][r_id] + \
        ...
        relation_range = [self.nodes['shift'][k] for k in range(
            len(self.nodes['shift']))] + [self.nodes['total']]
        for r_id in self.links_test['data'].keys():
    ...
    def get_test_neigh_w_random(self):
        ...
        for r_id in edge_types:
            ...
            pos_neigh[r_id], neg_neigh[r_id] = defaultdict(
    ...
    def get_test_neigh_full_random(self):
        ...
        for r_id in edge_types:
            ...
            test_label[r_id] = []
            h_type, t_type = self.links_test['meta'][r_id]
            h_range = (self.nodes['shift'][h_type], self.nodes['shift']
                       [h_type] + self.nodes['count'][h_type])
            t_range = (self.nodes['shift'][t_type], self.nodes['shift']
                       [t_type] + self.nodes['count'][t_type])
            (row, col), data = self.links_test['data'][r_id].nonzero(
            ), self.links_test['data'][r_id].data
    ...
    def load_links(self, is_test=False):
        """
        return links dict
        ...
        links = {'total': 0, 'count': Counter(), 'meta': {},
                 'data': defaultdict(list)}
        links_test = {'total': 0, 'count': Counter(), 'meta': {},
                      'data': defaultdict(list)}
        relations_path = self.path + '/relations.dict'
        entities_path = self.path + '/entities.dict'
        train_path = self.path + '/train.txt'
        valid_path = self.path + '/valid.txt'
        test_path = self.path + '/test.txt'

        train_triplets = read_file.read_triplets_as_list(
            train_path, entities_path, relations_path)
        valid_triplets = read_file.read_triplets_as_list(
            valid_path, entities_path, relations_path)
        test_triplets = read_file.read_triplets_as_list(
            test_path, entities_path, relations_path)
        ...
        for i in train_triplets:
            ...
            links['data'][r_id].append((h_id, t_id, 1.0))
            links['count'][r_id] += 1
            links['total'] += 1
            if r_id not in train_pos:
                train_pos[r_id] = [[], []]
        ...
        for i in test_triplets:
            h_id, r_id, t_id = i
            if r_id not in links_test['meta']:
                h_type = self.get_node_type(h_id)
                t_type = self.get_node_type(t_id)
                links_test['meta'][r_id] = (h_type, t_type)
        ...
        for r_id in links_test['data']:
            new_data_test[r_id] = self.list_to_sp_mat(links_test['data'][r_id])
        links_test['data'] = new_data_test
        return links, links_test, train_pos, valid_pos
