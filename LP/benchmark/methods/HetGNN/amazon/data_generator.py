...
class input_data(object):
    def __init__(self, args, dl):
        ...
        data_info = json.load(open("../../data/amazon/info.dat", 'r'))
        ...
        for node_type in node_types:
            for n_id in range(node_n[node_type]):
                ...
                if len(neigh_temp):
                    ...
                    while neigh_L < 30:  # maximum neighbor size = 100
            ...
            for n_id in range(self.node_n[node_type]):
                if len(self.neigh_list[node_type][n_id]):
                    ...
                    for l in range(self.args.walk_L - 1):
                        ...
                        if self.neigh_list[curNodeType][curNode]==[]:
                            curNode = self.node_type2name[curNodeType]+str(curNode)
                        else:
            ...
            for j in range(walk_L):
                ...
                if len(centerNode) > 1:
                    ...
                    for k in range(j - window, j + window + 1):
                        if k >= 0 and k < walk_L and k != j and k<len(path):
                            neighNode = path[k]
