...
class KGNN_LS(object):
    ...
    def _build_model(self, n_user, n_entity, n_relation):
        ...
        if model_type == 'gat':
            ...
            self.attn_drop = tf.placeholder(
                dtype=tf.float32, shape=(), name="attn_drop")
            self.ffd_drop = tf.placeholder(
                dtype=tf.float32, shape=(), name="ffd_drop")
            ...
            self.drop = tf.placeholder(dtype=tf.float32, shape=(), name="drop")
