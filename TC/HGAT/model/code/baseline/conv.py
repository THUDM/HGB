...
class myGATConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc_w = nn.Linear(1, num_heads, bias=False)
        self.attn_l = nn.Parameter(th.FloatTensor(size=(1, num_heads, out_feats)))
        self.attn_r = nn.Parameter(th.FloatTensor(size=(1, num_heads, out_feats)))
        self.attn_e = nn.Parameter(th.FloatTensor(size=(1, num_heads, edge_feats)))
        self.attn_w = nn.Parameter(th.FloatTensor(size=(1, num_heads, 1)))
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_normal_(self.fc_w.weight, gain=gain)
    ...
    def forward(self, graph, feat, e_feat, res_attn=None):
        with graph.local_scope():
            ...
            ew = (self.fc_w(graph.edata['weight'].view(-1, 1).float()).view(-1, self._num_heads, 1) * self.attn_w).sum(dim=-1).unsqueeze(-1)
            ee = (e_feat * self.attn_e).sum(dim=-1).unsqueeze(-1)
            el = (feat_src * self.attn_l).sum(dim=-1).unsqueeze(-1)
            er = (feat_dst * self.attn_r).sum(dim=-1).unsqueeze(-1)
            graph.srcdata.update({'ft': feat_src, 'el': el})
            graph.dstdata.update({'er': er})
            graph.edata.update({'ee': ee, 'ew': ew})
            graph.apply_edges(fn.u_add_v('el', 'er', 'e'))
            e = self.leaky_relu(graph.edata.pop('e')+graph.edata.pop('ee')+graph.edata.pop('ew'))
            # compute softmax
