...
def run_model_DBLP(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    #join_list = np.concatenate((train_idx, val_idx, test_idx), axis=None)
    #print(np.sort(join_list))
    #print(len(join_list))
    print(labels.size())
    ...
    #new_features = nn.Parameter(torch.empty(size=(N-N_p, D)))
    #nn.init.xavier_normal_(new_features)
    #new_features = torch.zeros(N-N_p, D).cuda()
