...
def build_relation_adj(org_graph, num_relations, rw_len=3, batch_size=3):
    ...
    print('generating random walks, rw_len={:03d}, batch_size={:03d}.'.format(rw_len, batch_size))
    walker = utils.RandomWalker(org_graph, rw_len, batch_size=batch_size)
    rws = walker.walk().__next__()
    rws_edge = utils.edge_path_from_random_walk(org_graph, rws)
    rws_edge = rws_edge - 1  # edge count from zero

    # Assemble score matrix from the random walks
    scores_matrix = utils.score_matrix_from_random_walks(rws_edge, num_relations).tocsr().tocoo()
...
def build_coarsened_line_graph(dataset, rw_len, batch_size, name):
    num_relations = dataset.num_relations
    org_graph = dataset[0]
    edge_index = org_graph.edge_index.numpy()
    edge_type = org_graph.edge_type.numpy() + 1
    row, col = edge_index
    org_adj = sp.coo_matrix((edge_type, (row, col)), shape=(org_graph.num_nodes, org_graph.num_nodes))
